...
export function detectChrome({ platform = process.platform, env = process.env, exists = fs.existsSync } = {}) {
  ...
  if (platform === "darwin") {
    const executable = MAC_LOCATIONS.find((candidate) => exists(candidate));
    return executable
      ? { available: true, executable, detected_by: "standard-macos-location" }
      : { available: false, executable: null, detected_by: null };
  }
  if (platform === "win32") {
    for (const variable of ["PROGRAMFILES", "PROGRAMFILES(X86)", "LOCALAPPDATA"]) {
      if (!env[variable]) continue;
      const executable = path.join(env[variable], "Google", "Chrome", "Application", "chrome.exe");
